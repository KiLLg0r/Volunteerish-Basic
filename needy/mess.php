...
?>
    ...
    <body>
        ...
        <div class="page-section page-3 active">
            <div class="messages">
                ...
                <div class="chat-list">
                    ...
                    $count = 0;
                    $usersCount = 0;
                    $users = "SELECT * FROM user WHERE status = 'Volunteer'";
                    $usersResults = mysqli_query($db, $users);
                    if (mysqli_num_rows($usersResults) > 0) {
                        while ($usersRow = mysqli_fetch_assoc($usersResults)) {
                            $usersCount++;
                            $volID = $usersRow['id'];
                            $msg = "SELECT * FROM messages WHERE (sent_by = '$id' AND received_by = '$volID') OR (sent_by = '$volID' AND received_by = '$id') ORDER BY createdAt DESC";
                            $msgResult = mysqli_query($db, $msg);
                            if (mysqli_num_rows($msgResult) > 0) {
                                $msgRow = mysqli_fetch_assoc($msgResult);
                    ?>
                                <div class="chat-list-row" onclick="location.href='../msg.php?sendID=<?= $id ?>&receiveID=<?= $volID ?>';">
        ...
        <footer id="btnContainer" class="btn-footer">
            ...
            <a class="link link-3 active" href="./mess.php">
                <div class="icon">
